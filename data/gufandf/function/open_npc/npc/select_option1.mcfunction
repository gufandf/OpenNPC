...
data modify storage gufandf:open_npc/chat temp.option_index set from entity @a[tag=temp_target,limit=1] SelectedItem.components."minecraft:custom_data".option
function gufandf:open_npc/npc/select_option2 with storage gufandf:open_npc/chat temp
execute as @a[tag=temp_target,limit=1] run function gufandf:open_npc/player/inventory/load
...
tag @a[tag=temp_target,limit=1] remove chooseing
tag @a[tag=temp_target,limit=1] remove dont_throw_item
tag @s add temp_end
# 多层有BUG
